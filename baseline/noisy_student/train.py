...
def parse_args():
    ...
    group_trainer.add_argument("--dropout", default=0.0, type=float, required=False,
                    help="dropout for noisy student")
...
def main():
    ...
    save_dir = os.path.join(args.output_dir, "_".join([os.path.basename(args.ptlm), 
        ...
        f"pseudo_lr{args.pseudo_lr}_decay{args.pseudo_lrdecay}_step{args.pseudo_decay_every}_evalstep{args.pseudo_eval_every}_dropout_{args.dropout}", # pseudo params
    ...
    # load model
    model = KGBERTClassifier(args.ptlm, args.dropout).to(args.device)
    ...
    best_val_score = 0
    early_steps = [10,30, 50, 75, 100, 125, 150, 175, 200]
    ...
    for phase in ['pseudo']:
        ...
        for e in range(num_epochs):
            ...
            for iteration, data in iterator:
                ...
                if phase == "pseudo":
                    y = data['soft_label']
                    y = torch.vstack((1-y, y)).transpose(0, 1).to(args.device, dtype=torch.float32)
                else:
                ...
                if phase != "pseudo": # or if torch.__version__ >= 1.10.0
                    loss = criterion(logits, y)
                else:
                    loss = torch.mean(torch.sum(-y * F.log_softmax(logits, dim=-1), -1))
