...
"""
Word level augmentations including Replace words with 
uniform random words or TF-IDF based word replacement.
"""
...
class TfIdfWordRep(EfficientRandomGen):
  ...
  def get_replace_prob(self, all_words):
    ...
    for word in all_words:
      cur_tf_idf[word] += 1. / len(all_words) * self.idf.get(word, 1)
    replace_prob = []
    for word in all_words:
